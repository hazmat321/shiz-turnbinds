...
|_||_|_||_|_|\__|\__/ |_|\__/\|___/ \__/|_|_\_|
    ...
    while (shouldRunSim) {
        if (_kbhit()) {
            ...
            if (ch == 224) {
                ch = _getch();
                if (ch == 72) selectedOption = (selectedOption - 1 + 8) % 8;
                else if (ch == 80) selectedOption = (selectedOption + 1) % 8;
                else if (ch == 75 || ch == 77) {
...
int selectedOption = 0;
    ...
    bool lastAutoToggleKeyDown = false; // For debouncing
    ...
    while (shouldRunSim) {
        if (_kbhit()) {
            ...
            if (ch == 224) {
                ch = _getch();
                if (ch == 72) selectedOption = (selectedOption - 1 + 9) % 9; // now 9 options
        ...
        if (autoToggleKeyDown && !lastAutoToggleKeyDown) {
            ...
            settings.save();
            ui.updateDebugDisplay(std::string("Auto-Activate toggled ") + (cfg.autoActivate ? "Off" : "On"));
            ui.updateSettingsDisplay(selectedOption, settings.get());
        }
